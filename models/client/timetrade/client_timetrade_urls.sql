...
sum(case when (first_seen = date and url like '%/resource/%') then 1 else 0 end) NewPages,
sum(case when (last_seen = date and url like '%/resource/%') then 1 else 0 end) RetiredPages
