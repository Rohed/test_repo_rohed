...
sum(case when (first_seen = date and url like 'blog.greenlight.guru%') then 1 else 0 end) NewPages,
sum(case when (last_seen = date and url like 'blog.greenlight.guru%' ) then 1 else 0 end) RetiredPages
FROM {{ref('agg_all_line_level')}}
WHERE client = 'Greenlight Guru'
AND ( goal_number in (1, 4, 8) OR goal_number is null )
