...
sum(case when (first_seen = date and url like 'pos.freedomgeneral.com%') then 1 else 0 end) NewPages,
sum(case when (last_seen = date and url like 'pos.freedomgeneral.com%') then 1 else 0 end) RetiredPages
FROM {{ref('agg_all_line_level')}}
WHERE client = 'Freedom National'
AND ( goal_number in (2, 5) OR goal_number is null )
